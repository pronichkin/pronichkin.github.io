...
Set-clusterNodeEx
{
 ...
 <# ,
             ...
             <# {
                ...
                'HP'
                {
                  # Connect to HP OneView
                    ...
                    $Message = "  Obtaining physical server topology from appliance `“$($applianceAddress.Host)`”"
                    Write-Verbose -Message $Message

                    $hpovMgmtParam = @{

                        Hostname        = $applianceAddress.Host
    ...
    Process
    {
       #region  1/5  Multi-node generic configuration

            $Message = '  Step 1/5  Multi-node generic configuration'
            Write-Verbose -Message $Message
       ...
       #region  2/5  Multi-node virtualization host configuration

            $Message = '  Step 2/5  Multi-node virtualization host configuration'
            Write-Verbose -Message $Message
                        ...
                        $Command | ForEach-Object -Process {
                            ...
                            )
                            {
                                $FileParam = @{
                                    ...
                                    InputObject = $PlatformIdentifier.InnerXml
                                    Encoding    = 'utf8'
                                }
                                Out-File @FileParam
                            }
       ...
       #region  3/5  Configure Hyper-V host in VMM

            $Message = '  Step 3/5  Per-machine virtualization host configuration in VMM'
            Write-Verbose -Message $Message
                       ...
                       #region Host properties
                            ...
                            )
                            {
                             ...
                             <# We no longer derive HGS URLs from VMM server
                                ...
                                $HgsAddressPrimary  = Resolve-DnsNameEx -Name $SiteProperty.HgsNamePrimary
                                $HgsAddressFallback = Resolve-DnsNameEx -Name $SiteProperty.HgsNameFallback
       ...
       #region  4/5  Configure Hyper-V host using Hyper-V native tools

            $Message = '  Step 4/5  Per-machine virtualization host configuration using native Hyper-V'
            Write-Verbose -Message $Message
                    ...
                    }
                    {
                       #region Host configuration
                            ...
                            $cimSessionCurrent = $cimSession | Where-Object -FilterScript {
                                $psItem.ComputerName -eq $NodeCurrent.ComputerName
                            }
       ...
       #region  5/5  Configure physical server in VMM

            $Message = '  Step 5/5  Per-machine virtualization host physical server metadata in VMM'
            Write-Verbose -Message $Message
       ...
       #endregion  5/5  Configure physical server in VMM
    }
