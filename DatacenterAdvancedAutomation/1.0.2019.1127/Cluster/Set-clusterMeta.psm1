                ...
                )
                {
                    ...
                    )
                    {
                      ...
                      # This should be done after volumes are created
                        ...
                        If
                        (
                            $ClusterPropertyCurrent[ 'BrokeripAddress' ]
                        )
                        {
                            $ReplicationServerParam.Add(

                                'ipAddress',
                                [System.Net.IPAddress]$ClusterPropertyCurrent.BrokeripAddress
                            )
                        }
                        Else
                        {
                            $StaticIPAddressPoolParam = @{
                            
                                ipAddress = $Cluster.ipAddresses[0].ipAddressToString
                                vmmServer = $vmmServer
                            }
                            $StaticIPAddressPool = Get-scStaticIPAddressPool @StaticIPAddressPoolParam

                            $ipAddressParam = @{
                                
                                StaticIPAddressPool = $StaticIPAddressPool
                                GrantToObjectType   = 'HostCluster'
                                GrantToObjectID     = $Cluster.ID
                                Description         = 'Hyper-V Replica Broker'
                            }
                            $ipAddress = Grant-scIpAddress @ipAddressParam

                            $ReplicationServerParam.Add(

                                'ipAddress',
                                [System.Net.IPAddress]$ipAddress.Address
                            )
                        }
