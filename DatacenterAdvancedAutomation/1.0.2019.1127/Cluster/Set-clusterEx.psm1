...
Set-clusterEx
{
    ...
    End
    {
      ...
      # 3072 or 4096 cause “The data is invalid” error on earlier functional levels
        $Cluster.ClusterLogSize              = 2048

      # Default “Config Store Root Path” for replicated data

        $Resource = Get-ClusterResource -InputObject $Cluster |
            Where-Object -FilterScript {
                $psItem.Name -eq 'Virtual Machine Cluster WMI'
            }
        ...
        If
        (
            $Resource -and $SharedVolume
        )
        {
            $PathParam = @{

                Path      = $SharedVolume.SharedVolumeInfo.FriendlyVolumeName
                ChildPath = 'Config Store Root'
            }
            $ConfigStoreRootPath = Join-Path @PathParam

            $ParameterParam = @{

                InputObject = $Resource
                Name        = 'ConfigStoreRootPath'
                Value       = $ConfigStoreRootPath
            }
            Set-ClusterParameter @ParameterParam
        }
        Else
        {
            $Message = "`“Virtual Machine Cluster WMI`” resource and/or Shared Volume(s) were not found, skipping `“Config Store Root`” configuration"
        ...
        {
            $Cluster = Get-scvmHostCluster -Name ( Resolve-dnsNameEx -Name $Cluster.Name )
        }
