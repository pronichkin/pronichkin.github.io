...
New-scTemplateEx
{
    ...
    Process
    {
        $Message = "Entering New-scTemplateEx for `“$Name`”"
            ...
            $NetworkAdapterGroupPropertyBmd | ForEach-Object -Process {
              ...
              # belongs to a VM Host Server
                ...
                )
                {
                    ...
                    $UplinkPortProfileSetParam = @{
                    
                        LogicalSwitch = $LogicalSwitch
                        Location      = $Location
                        Role          = $ScaleUnitType
                    }
                    $UplinkPortProfileSet = Get-scUplinkPortProfileSetEx @UplinkPortProfileSetParam
       ...
       #region Define the rest of Physical Computer Template
          ...
          # be blank.            
            ...
            $TemplateParam.GetEnumerator() | ForEach-Object -Process { 
       ...
       #region Add custom “Post-Deployment” script
            ...
            $ResourceName = "Clean $DiskCount Disks"

            $HostGroupParam = @{
            
                Name      = $VirtualHardDisk.LibraryGroup
                vmmServer = $vmmServer
            }

            $HostGroup = Get-scvmHostGroup @HostGroupParam

            $ResourceParam = @{
            
                Name         = $ResourceName
                Type         = 'CustomResource'
                LibraryGroup = $HostGroup
                vmmServer    = $vmmServer
            }
            $Resource = Get-scLibraryItemEx @ResourceParam
            ...
            )
            {
                ...
                $FolderName = $ResourceName + '.cr'

                $PathFolder = Join-Path -Path $env:temp -ChildPath $FolderName

                [System.Void]( New-Item -Path $PathFolder -ItemType 'Directory' )
                ...
                $LibraryServer = Set-scLibraryServer @LibraryServerParam
            
                $ResourceParam = @{
                
                    SourcePath               = $PathFolder
                    SharePath                = $LibraryShare.Path
                    OverwriteExistingFiles   = $True
                    AllowUnencryptedTransfer = $True
                }
                $Resource = Import-scLibraryPhysicalResource @ResourceParam

                $Description = "DiskPart script to be run in Windows PE in order to clear S2D leftovers on $DiskCount Physical Disks"
                ...
                $ResourceParam = @{
                    ...
                    Name           = $ResourceName
                    Description    = $Description
                    FamilyName     = $ResourceName
                    Release        = "1.0.0.$DiskCount"
                }
                $Resource = Set-scCustomResource @ResourceParam
            ...
            )
            {
                $Package | ForEach-Object -Process {
                    ...
                    $PackageCurrent  = $psItem
                    $PackagePath     = Join-Path -Path $SoftwareRoot.FullName -ChildPath $PackageCurrent.Name
                    $PackageFileInfo = Get-Item  -Path $PackagePath

                    Switch
                    (
                        $PackageFileInfo.Extension
                    )
                    {
                        '.msi'
                        {
                            ...
                            $CommandParameter += " ""$( $PackageFileInfo.FullName )"""

                            If
                            (
                                $PackageCurrent[ 'Property' ]
                            )
                            {
                                $PackageCurrent[ 'Property' ].GetEnumerator() | ForEach-Object -Process {

                                    If
                                    (
                                        $psItem.Value.ToString().StartsWith( '@' ) -and
                                        $psItem.Value.ToString().EndsWith( '@' )
                                    )

                                 <# This is the value which is specific to a host
                                    group and stored in a custom property of 
                                    either the current host group or the parent  #>

                                    {
                                        $PropertyName = $psItem.Value.TrimStart( '@' ).TrimEnd( '@' )
                                    ...
                                    $CommandParameter += " $( $psItem.Key )=$Value"
                                }                    
                            }
                        }
                        ...
                        '.exe'
                        {
                            $CommandPath = $PackageFileInfo.FullName
                        }
        ...
        $Message = "Exiting  New-scTemplateEx for `“$Name`”"
        Write-Verbose -Message $Message
