    ...
    Process
    {
        ...
        Else
        {
          ...
          # Replication Settings
            ...
            $FriendlyVolumeName = $SharedVolume.sharedVolumeInfo.FriendlyVolumeName

          # This is kinda weird format, but that's how Cluster stores Replica settings

            $AuthorizationValue = "* $FriendlyVolumeName\ DEFAULT"
          ...
          # Default “Config Store Root Path” for replicated data
            ...
            $ResourceWMI = Get-ClusterResource -InputObject $Cluster -Name 'Virtual Machine Cluster WMI'

            Set-ClusterParameter -InputObject $ResourceWMI -Name 'ConfigStoreRootPath' -Value $FriendlyVolumeName
