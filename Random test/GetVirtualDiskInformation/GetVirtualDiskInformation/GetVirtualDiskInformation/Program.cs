...
namespace ConsoleApplication1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // var result = OpenVirtualDisk(ref storageType, @"c:\Image\Child.vhdx", VirtualDiskAccessMask.None, OpenVirtualDiskFlag.None,
            var result = OpenVirtualDisk(ref storageType, @"C:\Image\Test01.vhdx", VirtualDiskAccessMask.None, OpenVirtualDiskFlag.None,
            ...
            GetVirtualDiskInfo info = new GetVirtualDiskInfo { Version = GetVirtualDiskInfoVersion.Identifier };
            Marshal.StructureToPtr(info, raw, true);

            var infoSize = (uint)320;//Marshal.SizeOf(info);
            ...
            int offset = Marshal.OffsetOf<GetVirtualDiskInfo>("Union").ToInt32();

            switch (info.Version)
            {
                case GetVirtualDiskInfoVersion.Size:
                    info.Union.Size = Marshal.PtrToStructure<GetVirtualDiskInfoSize>(raw + offset);
                    break;
                case GetVirtualDiskInfoVersion.Identifier:
                case GetVirtualDiskInfoVersion.ParentIdentifier:
                case GetVirtualDiskInfoVersion.VirtualDiskId:
                    info.Union.Identifier = Marshal.PtrToStructure<Guid>(raw + offset);
                    Console.WriteLine(info.Union.Identifier);
                    Console.ReadLine();
                    break;
                case GetVirtualDiskInfoVersion.ParentLocation:
                    info.Union.ParentLocation.ParentResolved = Marshal.ReadInt32(raw + offset) != 0;
                    // you'll have to decide what to do with this parameter
                    // string parentLocationBuffer = Marshal.PtrToStringUni(raw + offset + 4);
                    break;
                case GetVirtualDiskInfoVersion.ParentTimestamp:
                case GetVirtualDiskInfoVersion.ProviderSubtype:
                case GetVirtualDiskInfoVersion.VhdPhysicalSectorSize:
                case GetVirtualDiskInfoVersion.Fragmentation:
                    info.Union.ParentTimestamp = (uint)Marshal.ReadInt32(raw + offset);
                    break;
                case GetVirtualDiskInfoVersion.VirtualStorageType:
                    info.Union.VirtualStorageType = Marshal.PtrToStructure<VirtualStorageType>(raw + offset);
                    break;
                case GetVirtualDiskInfoVersion.Is4KAligned:
                case GetVirtualDiskInfoVersion.IsLoaded:
                    info.Union.Is4kAligned = Marshal.ReadInt32(raw + offset) != 0;
                    break;
                case GetVirtualDiskInfoVersion.PhysicalDisk:
                    info.Union.PhysicalDisk = Marshal.PtrToStructure<GetVirtualDiskInfoPhysicalDisk>(raw + offset);
                    break;
                case GetVirtualDiskInfoVersion.SmallestSafeVirtualSize:
                    info.Union.SmallestSafeVirtualSize = (ulong)Marshal.ReadInt64(raw + offset);
                    break;
                case GetVirtualDiskInfoVersion.ChangeTrackingState:
                    // info.Union.ChangeTrackingState.Enabled = Marshal.ReadInt32(raw + offset) != 0;
                    // info.Union.ChangeTrackingState.NewerChanges = Marshal.ReadInt32(raw + offset + 4) != 0;
                    string mostRecentId = Marshal.PtrToStringUni(raw + offset + 8);
                    break;
            }
