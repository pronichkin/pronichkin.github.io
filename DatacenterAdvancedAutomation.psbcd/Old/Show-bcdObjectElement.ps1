...
Show-bcdObjectElement
{
    ...
    $ObjectElement | ForEach-Object -Process {
        
        $Element = $PSItem

        $Type = $ElementType.GetEnumerator() | Where-Object { $PSItem.Value -eq $Element.Type }

        Switch
        (
            $Element.CimClass.CimClassName
        )
        {
            "BcdStringElement"
            {
